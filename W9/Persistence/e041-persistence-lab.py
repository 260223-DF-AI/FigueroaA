...
def pii_middleware_node(state: AgentState):
    ...
    state["messages"][-1].content = re.sub(pattern, '[REDACTED]', state["messages"][-1].content)
...
def model_node(state: AgentState):
    ...
    init = ChatBedrock(model="us.anthropic.claude-sonnet-4-20250514-v1:0")
    response = init.invoke(state["messages"])
    return {'messages': [response]}
...
def run_exercise():
    ...
    print(graph.invoke(s2, config=config)["messages"][-1].content)
